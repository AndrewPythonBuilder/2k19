...
part = ''
otvet = ''
answeri = var =answeris = sal = raztel = q = texts = answeri1 = q1 = raz_ph = next_ = rzdl = ss = only_text1 = only_text = only_text_op1 = only_text_op = False
...
def start(bot, update):
    ...
    global part, raztel, text, photo, answeri, var, ans, answeris, sal, q, texts, answeri1, q1, raz_ph, next_, rzdl, otvet, ss, only_text1, only_text, only_text_op, only_text_op1
    answeri = var = answeris = sal = raztel = q = texts = answeri1 = q1 = raz_ph = next_ = rzdl = ss = only_text1 = only_text = only_text_op1 = only_text_op = False
...
def answer_questions(bot, update):
    ...
    if (message.contact != None) and (otvet == ''):
        ...
        if base_w.set_razdel() != None:
            bottons.append(['Получить скидку'])
        keyboard = ReplyKeyboardMarkup(bottons, resize_keyboard = True)
        bot.send_message(message.chat.id, 'Спасибо, ваш контакт отправлен, оператор в ближайшее время спишется с вами', reply_markup = keyboard)
        ...
        for i in te:
            if "####" in i:
                vopros.append(i.split('####')[2])
        for i in range(len(ot)-1):
            txt += '''Вопрос: %s \nОтвет: %s ''' %(vopros[i], ot[i])
        for i in constants.admins:
            try:
                bot.send_message(i, 'Имя: '+ str(message.chat.username) +'\nНомер телефона: '+ str(base_w.select_number(message.chat.id))+ '\n'+ str(txt)+ '\nВремя заполнения: ' + str(now.strftime("%d-%m-%Y %H:%M")))# ТААААААК ЕС ЧО ТЕСТИТЬ И ИСПРАВИТЬ!
            except:
                pass
        otvet = ''
        ...
        bot.send_message(message.chat.id, 'Выберите скидку, который вы хотите включить. \nКак только вы включите один скидку, другой, который был включен до этого, выключится', reply_markup=keyboard)
        rzdl = True
    elif rzdl == True:
        rzdl = False
        if message.text == 'Отмена':
            ...
            try:
                bot.send_message(message.chat.id, 'Ваша скидка включен, теперь у пользователей появилась кнопка "Получить скидку"')
...
def button(bot, update):
    ...
    try:
        ...
        if str(query.data).split('+')[0] =='sale':
            ...
            bot.edit_message_caption(message_id=query.message.message_id, caption="Ваш ответ: %s" %(str(query.data).split('+')[1]),
